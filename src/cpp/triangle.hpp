...
public:
  ...
  const int toBC;
  const int toCA;
  Triangle(CPP_Point3 A, CPP_Point3 B, CPP_Point3 C,
   ...
   **/
  calc_percent_result calc_percent_gnomonic(const CPP_Point3 &p) const;
   ...
   * @returns percents along CA and CB
   **/
  calc_percent_result calc_percent_quaternion(const CPP_Point3 &p) const;
  ...
  all_points(int res, ico::rotation_method rm) const;
  /**
  ...
  lazy_points_around(CPP_Point3 &p, int res,
  ...
  CPP_Point3 generate_point(int res, int lower_vert, int lower_horz,
                            ico::rotation_method rotation) const;

  /**
   * @param point point to test
   * @returns whether triangle contains point */
  bool contains_point(CPP_Point3 &point) const;

  /**
   * @param vec vector from origin to point
   * @returns point where [vec] intersects with this triangle's plane */
  CPP_Point3 plane_intersection(CPP_Point3 vec) const;
